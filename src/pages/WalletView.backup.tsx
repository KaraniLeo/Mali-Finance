...
export function WalletView() {
  ...
  const handleDeposit = async () => {
    ...
    // Auto-allocate based on budget rules using backend logic
    await useWalletStore.getState().distributeDeposit(amount, 'Manual Wallet Deposit');

    toast.success('Funds deposited and smartly allocated!');
  };
              ...
              <div className="flex bg-white/10 rounded-xl overflow-hidden p-1 backdrop-blur-sm border border-white/20">
                ...
                <button 
                  onClick={() => {
                    ...
                    setDepositAmount('');
                    useWalletStore.getState().updateWalletBalance(nb);
                    addTransaction({ id: Date.now().toString(), wallet_id: walletId || 'local', amount, type: 'debit', description: 'Manual Subtraction', created_at: new Date().toISOString() });
                    toast.success('Funds subtracted!');
                  }}
