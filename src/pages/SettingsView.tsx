...
export function SettingsView({ user, onLogout }: SettingsViewProps) {
  ...
  const [selectedThemeTier, setSelectedThemeTier] = useState<'theme-pro' | 'theme-teen' | 'theme-junior'>(() => {
    return (localStorage.getItem(`theme-tier-${user.id}`) as any) || `theme-${user.tier}` || 'theme-pro';
  });

  const handleThemeTierChange = (newTier: 'theme-pro' | 'theme-teen' | 'theme-junior') => {
    setSelectedThemeTier(newTier);
    localStorage.setItem(`theme-tier-${user.id}`, newTier);
    const root = document.documentElement;
    root.classList.remove('theme-junior', 'theme-teen', 'theme-pro');
    root.classList.add(newTier);
  };
  ...
  if (activeRoute === 'profile') {
    return (
      <div className="flex flex-col gap-6 h-full overflow-hidden dark:text-white">
        ...
        <div>
          <h2 className="text-3xl font-bold text-brand-secondary dark:text-brand-primary brand">Profile Information</h2>
        ...
        <div className="bg-white dark:bg-stone-800 p-8 rounded-3xl border border-stone-100 dark:border-stone-700 shadow-sm max-w-2xl">
          <div className="space-y-6">
             ...
             <button className="px-6 py-3 bg-brand-accent hover:bg-[#5A7A1B] text-white font-bold rounded-xl transition-colors shadow-lg shadow-[#6B8E23]/20">Save Changes</button>
           </div>
        </div>
      </div>
    );
  ...
  if (activeRoute === 'notifications') {
    return (
      <div className="flex flex-col gap-6 h-full overflow-hidden dark:text-white">
        ...
        <div>
          <h2 className="text-3xl font-bold text-brand-secondary dark:text-brand-primary brand">Notifications</h2>
        ...
        <div className="bg-white dark:bg-stone-800 p-6 rounded-3xl border border-stone-100 dark:border-stone-700 shadow-sm max-w-2xl space-y-4">
           <div className="flex items-center justify-between p-4 bg-stone-50 dark:bg-stone-700/50 rounded-2xl">
             ...
             <button onClick={() => setNotifications(!notifications)} className={`w-12 h-6 rounded-full relative p-1 transition-colors ${notifications ? 'bg-brand-accent' : 'bg-stone-300 dark:bg-stone-600'}`}>
  ...
  if (activeRoute === 'privacy') {
    return (
      <div className="flex flex-col gap-6 h-full overflow-hidden dark:text-white">
        ...
        <div>
          <h2 className="text-3xl font-bold text-brand-secondary dark:text-brand-primary brand">Privacy & Terms</h2>
        ...
        <div className="bg-white dark:bg-stone-800 p-8 rounded-3xl border border-stone-100 dark:border-stone-700 shadow-sm max-w-2xl overflow-y-auto">
           <div className="prose dark:prose-invert prose-stone max-w-none text-stone-700 dark:text-stone-300">
             <h3 className="text-xl font-bold text-brand-secondary dark:text-brand-primary mb-2">1. Data Collection</h3>
             ...
             <h3 className="text-xl font-bold text-brand-secondary dark:text-brand-primary mb-2">2. Guardian Access</h3>
  ...
  return (
    <div className="flex flex-col gap-8 h-full overflow-hidden dark:text-white">
      <div>
        <h2 className="text-3xl font-bold text-brand-secondary dark:text-brand-primary brand">Settings</h2>
          ...
          <section>
            ...
            <div className="bg-white dark:bg-stone-800 border border-stone-100 dark:border-stone-700 rounded-3xl overflow-hidden shadow-sm">
              <div className="flex items-center justify-between p-6 hover:bg-stone-50 dark:hover:bg-stone-700 transition-all">
                ...
                <button 
                  onClick={toggleTheme}
                  className={`w-12 h-6 rounded-full relative p-1 transition-colors ${theme === 'dark' ? 'bg-brand-accent' : 'bg-stone-300 dark:bg-stone-600'}`}
                >
                  <motion.div 
              ...
              {/* Theme Tier Swatches Override */}
              <div className="p-6 border-t border-stone-50 dark:border-stone-700">
                <span className="font-bold text-xs uppercase tracking-wider text-stone-450 dark:text-stone-400 block mb-3">Theme Accent Colors</span>
                <div className="flex gap-4">
                  {[
                    { id: 'theme-pro', colors: ['#047857', '#D4A017', '#0F172A'], label: 'Pro theme' },
                    { id: 'theme-teen', colors: ['#2563EB', '#8B5CF6', '#14B8A6'], label: 'Teen theme' },
                    { id: 'theme-junior', colors: ['#4CC9F0', '#F9C74F', '#43AA8B'], label: 'Junior theme' }
                  ].map(sw => {
                    const isActive = selectedThemeTier === sw.id;
                    return (
                      <button
                        key={sw.id}
                        onClick={() => handleThemeTierChange(sw.id as any)}
                        title={sw.label}
                        className={`flex items-center gap-1.5 p-2 rounded-2xl border-2 transition-all cursor-pointer hover:scale-105 active:scale-95 ${
                          isActive 
                            ? 'border-brand-accent bg-stone-50 dark:bg-stone-700/50' 
                      ...
                      >
                        {sw.colors.map((c, i) => (
                          <div 
                            key={i} 
                            className="w-5 h-5 rounded-full border border-white dark:border-stone-850" 
