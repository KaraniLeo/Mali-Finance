...
export function Sidebar({ user, activeView, onViewChange, mobileMenuOpen, onLogout }: SidebarProps) {
  return (
    <nav className={`w-64 flex-shrink-0 bg-stone-100 border-r border-stone-200 flex flex-col p-6 absolute lg:relative inset-y-0 left-0 z-50 transition-transform lg:translate-x-0 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="hidden lg:flex items-center gap-2 mb-10">
        ...
        <span className="text-2xl font-bold tracking-tight brand text-[#2D3911]">MALI</span>
      </div>
      ...
      <div className="mt-auto pt-6 flex flex-col gap-4">
        <div className="p-4 rounded-2xl bg-stone-200/50 flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-emerald-300 border-2 border-white shadow-sm overflow-hidden flex-shrink-0">
            <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user.name}`} alt="Avatar" />
          </div>
          <div className="min-w-0">
            <div className="text-sm font-bold truncate">{user.name}</div>
            <div className="text-[10px] text-stone-500 font-semibold uppercase tracking-wider truncate">{user.tier} Explorer</div>
          </div>
        </div>
        <button 
          onClick={onLogout}
